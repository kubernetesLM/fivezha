...
project="yunpay_admin"
project_dir=$web_dir/tomcat_$project
web_url=http://manage.ipaye.cn/AtHome/login.jsp
case_yunpay
...
fail_ation(){
	...
	hm=$(date +%H%M)
	#1.记录日志
	#tail -n20000 $project_dir/logs/catalina.out > $log_bak_dir/${project}_catalina_${ymd}_$hm.out
